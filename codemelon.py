    ...
    def make_tokens(self):
        ...
        while self.current_char is not None:
            if self.current_char in ' \t':
                ...
                tokens.append(Token(TT_INTDIV, pos_start=self.pos))
                self.advance()
            elif self.current_char == '%':
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_input(self, exec_ctx):
        print(str(exec_ctx.symbol_table.get('message')))
        text = input()
    ...
    def execute_get_type(self, exec_ctx):
        item = exec_ctx.symbol_table.get('item')
        if (isinstance(item, Number) or isinstance(item, List) or isinstance(item, String) or
                isinstance(item, Function) or isinstance(item, BuiltInFunction)):
            return RTResult().success(String(type(item)))
        return RTResult().failure(RTError(
            self.pos_start, self.pos_end,
            'Given argument is not of a valid type',
            exec_ctx
        ))

    execute_get_type.arg_names = ['item']
    ...
    def execute_math_sin(self, exec_ctx):
        ...
        return RTResult().success(Number(math.sin(math.degrees(value.value))))
    ...
    def execute_math_cos(self, exec_ctx):
        ...
        return RTResult().success(Number(math.cos(math.degrees(value.value))))
    ...
    def execute_math_tan(self, exec_ctx):
        ...
        return RTResult().success(Number(math.tan(math.degrees(value.value))))
    ...
    execute_mod.arg_names = ['value1', 'value2']
...
BuiltInFunction.get_type = BuiltInFunction('get_type')
...
global_symbol_table.set('get_type', BuiltInFunction.get_type)
